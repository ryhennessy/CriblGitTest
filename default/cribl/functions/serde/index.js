...
exports.init = (opt) => {
  ...
  if (conf.type === 'elff') {
    ...
    serde = C.Text._kvpSerDe(fieldFilter, conf.cleanFields, conf.allowedKeyChars, conf.allowedValueChars);
  } else if (conf.type === 'json') {
    serde = C.Text._jsonSerDe(fieldFilter);
  } else if (conf.type === 'regex') {
    if (C.Text._regexSerDe == null) {
      throw new Error(`type =${conf.type} not supported`)
    }
    serde = C.Text._regexSerDe(conf);
  } else if (conf.type === 'grok') {
    if (C.Text._grokSerDe == null) {
      throw new Error(`type =${conf.type} not supported`)
    }
    serde = C.Text._grokSerDe(conf, C.util.GrokRule);
  } else {
